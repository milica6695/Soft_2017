...
def iseci_broj(broj):
    ...
    isecena = broj[y-2:y + h + 2, x-2:x + w + 2]
...
klasifikator.load_weights(''
                          'weights.h5')
#####
slike = ["frames/video-0.avi188.png","frames/video-1.avi0.png", 'frames/video-3.avi0.png']
#slika = cv2.imread("frames/video-0.avi188.png")     # 8
iNDEX = 0
licence_plate = ''
while iNDEX < len(slike):
    upper_red = np.array([255, 100, 100])
    lower_red = np.array([90, 0, 0])

    slikaa= cv2.imread(slike[iNDEX])

    mask = cv2.inRange(slikaa, lower_red, upper_red)
    output = cv2.bitwise_and(slikaa, slikaa, mask=mask)
    plt.imshow(output)
    plt.show()
    ...
    beleKonture = kontureBrojeva(slikaa)
    ...
    for contour in contours:  # za svaku konturu
        ...
        if povrsina > 12 and xx > x1 and width >= 2 and height > 7:  # uslov da kontura pripada (trebalo bi preko Krugova)                                yyy:yyy + hhh, xxx:xxx + www
            ...
            if sumiranje:
                suma = suma +  element
                sumiranje = False
                sumaElemenata.append(str(suma))
        # cv2.minAreaRect(contour)
    print('Suma identifikovanih brojeva : ' + str(suma) )

    licence_plate = licence_plate + str(slike[iNDEX])[0:18] + '     Suma je:' + str(suma) + '\n'
    print(licence_plate)
    iNDEX += 1
